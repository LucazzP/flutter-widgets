...
class PdfComboBoxField extends PdfListField {
  ...
  bool get editable {
    if (_helper.isLoadedField) {
      _editable = _helper.isFlagPresent(FieldFlags.edit) || _helper.flags.contains(FieldFlags.edit);
    }
    return _editable;
  }
  ...
  String get selectedValue => _helper.selectedValues.isEmpty ? '' : _helper.selectedValues[0];
...
class PdfComboBoxFieldHelper extends PdfListFieldHelper {
  ...
  /// internal method
  static PdfComboBoxField loadComboBox(PdfDictionary dictionary, PdfCrossTable crossTable) {
  ...
  void drawAppearance(PdfTemplate template) {
    ...
    final PaintParams params = PaintParams(
        bounds: Rect.fromLTWH(0, 0, comboBoxField.bounds.width, comboBoxField.bounds.height),
        backBrush: backBrush,
        foreBrush: foreBrush,
        borderPen: borderPen,
    ...
    if (comboBoxField.selectedIndex != -1 &&
        ...
        PdfDocumentHelper.getHelper(PdfPageHelper.getHelper(comboBoxField.page!).document!)
                .conformanceLevel ==
      ...
      template.graphics!.drawString(comboBoxField.items[comboBoxField.selectedIndex].text,
          comboBoxField.font ?? PdfStandardFont(PdfFontFamily.timesRoman, 12),
          brush: params.foreBrush, bounds: rectangle, format: format);
    }
  }
  ...
  void _applyAppearance(PdfDictionary widget) {
    ...
    } else if (comboBoxField.form!.readOnly == true || comboBoxField.readOnly == true) {
      PdfFormHelper.getHelper(comboBoxField.form!).setAppearanceDictionary = true;
    } else if (PdfFormHelper.getHelper(comboBoxField.form!).setAppearanceDictionary &&
        !PdfFormHelper.getHelper(comboBoxField.form!).needAppearances!) {
  ...
  void draw() {
    ...
    if (!isLoadedField && PdfAnnotationHelper.getHelper(widget!).appearance != null) {
      comboBoxField.page!.graphics
          .drawPdfTemplate(widget!.appearance.normal, comboBoxField.bounds.topLeft);
    } else {
      final Rect rect =
          Rect.fromLTWH(0, 0, comboBoxField.bounds.width, comboBoxField.bounds.height);
      ...
      if (comboBoxField.selectedIndex != -1) {
        ...
        if (comboBoxField.selectedIndex == -1 &&
          ...
          if (dictionary![PdfDictionaryProperties.dv] is PdfString) {
            text = (dictionary![PdfDictionaryProperties.dv]! as PdfString).value;
          } else {
      ...
      if (!isLoadedField) {
        ...
        final bool padding =
            parameters.style == PdfBorderStyle.inset || parameters.style == PdfBorderStyle.beveled;
        ...
        final Rect itemTextBound = Rect.fromLTWH(point.dx, point.dy, width - point.dx,
            parameters.bounds!.height - (padding ? doubleBorderWidth : borderWidth));
        template.graphics!
            .drawString(text!, font, brush: foreBrush, bounds: itemTextBound, format: format);
        comboBoxField.page!.graphics
        ...
        FieldPainter()
            .drawComboBox(comboBoxField.page!.graphics, prms, text, gp.font, gp.stringFormat);
      }
    }
  }
  ...
  void _drawComboBox(PdfGraphics? graphics) {
    final GraphicsProperties gp = GraphicsProperties(comboBoxField);
    gp.bounds = Rect.fromLTWH(0, 0, comboBoxField.bounds.width, comboBoxField.bounds.height);
    final PaintParams prms = PaintParams(
        bounds: gp.bounds,
        backBrush: gp.backBrush,
        foreBrush: gp.foreBrush,
        ...
        shadowBrush: gp.shadowBrush);
    String? text;
    if (selectedItems.count > 0 && comboBoxField.selectedIndex != -1 && !flattenField) {
      text = selectedItems[0].text;
    } else if (dictionary!.containsKey(PdfDictionaryProperties.dv) && !flattenField) {
    ...
    if (selectedItems.count == 0) {
      FieldPainter()
          .drawComboBox(graphics!, prms, comboBoxField.selectedValue, gp.font, gp.stringFormat);
    } else if (text != null && !flattenField) {
      FieldPainter().drawComboBox(graphics!, prms, text, gp.font, gp.stringFormat);
    } else {
  ...
  double getFontHeight(PdfFontFamily family) {
    ...
    if (comboBoxField.selectedIndex != -1) {
      ...
      widths.add(itemFont.measureString(comboBoxField.selectedItem!.text).width);
    } else {
      ...
      for (int i = 1; i < comboBoxField.items.count; ++i) {
        final double value = sfont.measureString(comboBoxField.items[i].text).width;
    ...
    double s = widths.isNotEmpty
        ? ((12 * (comboBoxField.bounds.size.width - 4 * comboBoxField.borderWidth)) /
    ...
    if (comboBoxField.selectedIndex != -1) {
      ...
      if (textSize.width > comboBoxField.bounds.width ||
        ...
        final double width = comboBoxField.bounds.width - 4 * comboBoxField.borderWidth;
        final double h = comboBoxField.bounds.height - 4 * comboBoxField.borderWidth;
        ...
        for (double i = 1; i <= comboBoxField.bounds.height; i++) {
          ...
          if (textSize.width > comboBoxField.bounds.width || textSize.height > h) {
            fontSize = i;
            do {
              fontSize = fontSize - 0.001;
              PdfFontHelper.getHelper(font).setSize(fontSize);
              final double textWidth = PdfFontHelper.getLineWidth(font, text, format);
