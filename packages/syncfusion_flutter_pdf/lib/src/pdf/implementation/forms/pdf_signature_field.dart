  ...
  PdfSignatureField(PdfPage page, String name,
    ...
    form!.fieldAutoNaming
        ? PdfAnnotationHelper.getHelper(_helper.widget!)
            .dictionary!
            .setProperty(PdfDictionaryProperties.ft, PdfName(PdfDictionaryProperties.sig))
        : _helper.dictionary!
            .setProperty(PdfDictionaryProperties.ft, PdfName(PdfDictionaryProperties.sig));
    ...
    if (borderWidth != null || borderColor != null || backColor != null || borderStyle != null) {
  ...
  void _initializeSignature(PdfSignature? value) {
    if (value != null) {
      ...
      PdfSignatureHelper.getHelper(_signature!).document =
          PdfPageHelper.getHelper(PdfSignatureHelper.getHelper(_signature!).page!).document;
      PdfSignatureHelper.getHelper(_signature!)
          .checkAnnotationElementsContainsSignature(page!, name);
      PdfSignatureHelper.getHelper(_signature!).field = this;
      PdfDocumentHelper.getHelper(PdfSignatureHelper.getHelper(_signature!).document!)
          .catalog
          .beginSave = PdfSignatureHelper.getHelper(_signature!).catalogBeginSave;
      ...
      if (!_helper.skipKidsCertificate) {
        ...
        if (_helper.isLoadedField) {
          ...
          final PdfDictionary widget =
              _helper.getWidgetAnnotation(_helper.dictionary!, _helper.crossTable);
  ...
  void _setSignature(IPdfPrimitive? signature) {
    ...
    if (signature is PdfReferenceHolder &&
      ...
      if (signatureDictionary.containsKey(PdfDictionaryProperties.subFilter)) {
        final IPdfPrimitive? filter =
            PdfCrossTable.dereference(signatureDictionary[PdfDictionaryProperties.subFilter]);
            ...
            if (tempDictionary != null && tempDictionary is PdfDictionary) {
              if (tempDictionary.containsKey(PdfDictionaryProperties.data)) {
                ...
                if (tempReferenceHolder != null &&
                    ...
                    !mainObjectCollection.containsReference(tempReferenceHolder.reference!)) {
                  final IPdfPrimitive? tempObject = mainObjectCollection
                      .objectCollection![tempReferenceHolder.reference!.objectCollectionIndex!]
                      .object;
      ...
      if (signatureDictionary.containsKey(PdfDictionaryProperties.m) &&
        ...
        PdfSignatureHelper.getHelper(_signature!).dateOfSign = _helper.dictionary!
            .getDateTime(signatureDictionary[PdfDictionaryProperties.m]! as PdfString);
      }
      if (signatureDictionary.containsKey(PdfDictionaryProperties.name) &&
        ...
        _signature!.signedName =
            (signatureDictionary[PdfDictionaryProperties.name]! as PdfString).value;
      }
      if (signatureDictionary.containsKey(PdfDictionaryProperties.reason)) {
        final IPdfPrimitive? reason =
            PdfCrossTable.dereference(signatureDictionary[PdfDictionaryProperties.reason]);
      ...
      if (signatureDictionary.containsKey(PdfDictionaryProperties.location)) {
        final IPdfPrimitive? location =
            PdfCrossTable.dereference(signatureDictionary[PdfDictionaryProperties.location]);
      ...
      if (signatureDictionary.containsKey(PdfDictionaryProperties.contactInfo)) {
        final IPdfPrimitive? contactInfo =
            PdfCrossTable.dereference(signatureDictionary[PdfDictionaryProperties.contactInfo]);
          ...
          if (catalog.containsKey(PdfDictionaryProperties.perms)) {
            ...
            if (catalogDictionary != null &&
              ...
              final IPdfPrimitive? permissionDictionary =
                  (docPermission is PdfReferenceHolder) ? docPermission.object : docPermission;
              if (permissionDictionary != null &&
                  permissionDictionary is PdfDictionary &&
                  permissionDictionary.containsKey(PdfDictionaryProperties.byteRange)) {
                ...
                if (byteRange != null &&
                    ...
                    _signature != null &&
                    PdfSignatureHelper.getHelper(_signature!).byteRange != null) {
            ...
            if (reference != null &&
              ...
              if (reference is PdfDictionary && reference.containsKey(PdfDictionaryProperties.p)) {
                final IPdfPrimitive? permissionNumber =
                    PdfCrossTable.dereference(reference[PdfDictionaryProperties.p]);
                if (permissionNumber != null && permissionNumber is PdfNumber) {
                  _signature!.documentPermissions = PdfSignatureHelper.getHelper(_signature!)
...
class PdfSignatureFieldHelper extends PdfFieldHelper {
  ...
  void draw() {
    if (!isLoadedField) {
      if (PdfAnnotationHelper.getHelper(widget!).appearance == null && appearance) {
        PdfAnnotationHelper.getHelper(widget!).appearance = PdfAppearance(widget!);
        PdfAnnotationHelper.getHelper(widget!).appearance!.normal =
            PdfTemplate(signatureField.bounds.width, signatureField.bounds.height);
        drawAppearance(PdfAnnotationHelper.getHelper(widget!).appearance!.normal);
      }
      super.draw();
      if (PdfAnnotationHelper.getHelper(widget!).appearance != null) {
        signatureField.page!.graphics
            .drawPdfTemplate(widget!.appearance.normal, signatureField.bounds.topLeft);
      }
    } else if (flattenField) {
      if (!isLoadedSign &&
          PdfAppearanceHelper.getHelper(signatureField.appearance).templateNormal != null) {
    ...
    if (content.containsKey(PdfDictionaryProperties.matrix)) {
      ...
      if (matrix != null && matrix is PdfArray) {
        final int angle = _obtainGraphicsRotation(
            (matrix.elements[2]! as PdfNumber).value!, (matrix.elements[0]! as PdfNumber).value!);
        if (angle != 0) {
          ...
          if (angle == 90) {
            ...
            rotatedTemplate.graphics!.drawPdfTemplate(
                template, Offset.zero, Size(template.size.height, template.size.width));
          } else if (angle == 180) {
            ...
            rotatedTemplate.graphics!.drawPdfTemplate(template, Offset.zero, template.size);
          } else if (angle == 270) {
            rotatedTemplate.graphics!.translateTransform(template.size.width, 0);
  ...
  void drawAppearance(PdfTemplate template) {
    ...
    if (appearance) {
      final PaintParams params = PaintParams(
          bounds: Rect.fromLTWH(0, 0, signatureField.bounds.width, signatureField.bounds.height),
          backBrush: backBrush,
          foreBrush: foreBrush,
          borderPen: borderPen,
          style: signatureField.borderStyle,
      ...
      FieldPainter().drawSignature(
          ...
          PaintParams(
              bounds:
                  Rect.fromLTWH(0, 0, signatureField.bounds.width, signatureField.bounds.height)));
    }
  }
  ...
  void beginSave() {
    if (!isLoadedSign &&
        appearance &&
        PdfAppearanceHelper.getHelper(signatureField.appearance).templateNormal == null) {
      signatureField.appearance.normal =
          PdfTemplate(signatureField.bounds.width, signatureField.bounds.height);
