...
class PdfTextMarkupAnnotation extends PdfAnnotation {
  ...
  PdfTextMarkupAnnotation(Rect bounds, String text, PdfColor color,
    ...
    _helper = PdfTextMarkupAnnotationHelper(this, bounds, text, color, author, subject, opacity,
        modifiedDate, setAppearance, flags, textMarkupAnnotationType);
    if (boundsCollection != null) {
      this.boundsCollection = boundsCollection;
    }
  }

  PdfTextMarkupAnnotation._(PdfDictionary dictionary, PdfCrossTable crossTable) {
  ...
  PdfTextMarkupAnnotationType get textMarkupAnnotationType => _helper.textMarkupAnnotationType;
...
class PdfTextMarkupAnnotationHelper extends PdfAnnotationHelper {
  ...
  PdfTextMarkupAnnotationHelper._(
      this.textMarkupAnnotation, PdfDictionary dictionary, PdfCrossTable crossTable)
      : super(textMarkupAnnotation) {
  ...
  /// internal method
  static PdfTextMarkupAnnotationHelper getHelper(PdfTextMarkupAnnotation annotation) {
    return annotation._helper;
  }

  /// internal method
  static PdfTextMarkupAnnotation load(PdfDictionary dictionary, PdfCrossTable crossTable) {
  ...
  PdfTextMarkupAnnotationType get textMarkupAnnotationType =>
      isLoadedAnnotation ? _obtainTextMarkupAnnotationType() : _textMarkupAnnotationType!;
  ...
  void setQuadPoints(Size pageSize) {
    if (!isLoadedAnnotation) {
      ...
      for (int i = 0; i < noofRect; i++) {
        final double locationX = _boundsCollection[i].left, locationY = _boundsCollection[i].top;
        ...
        textQuadLocation[5 + (i * 8)] = textQuadLocation[1 + (i * 8)] - _boundsCollection[i].height;
        textQuadLocation[6 + (i * 8)] = (locationX + _boundsCollection[i].width) + margins.left;
      ...
      final List<double> textQuadLocation = List<double>.filled(_boundsCollection.length * 8, 0);
      final double pageHeight = pageSize.height;
      ...
      dictionary!.setProperty(PdfDictionaryProperties.quadPoints, PdfArray(textQuadLocation));
    }
  }
  ...
  List<Rect> _obtainBoundsValue() {
    ...
    if (dictionary!.containsKey(PdfDictionaryProperties.quadPoints)) {
      final IPdfPrimitive? points =
          PdfCrossTable.dereference(dictionary![PdfDictionaryProperties.quadPoints]);
      double x, y, width, height;
      if (points != null && points is PdfArray) {
        for (int i = 0; i < (points.count / 8).round(); i++) {
          final double q1 = (points[0 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q2 = (points[1 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q3 = (points[2 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q4 = (points[3 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q5 = (points[4 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q6 = (points[5 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q7 = (points[6 + (i * 8)]! as PdfNumber).value!.toDouble();
          final double q8 = (points[7 + (i * 8)]! as PdfNumber).value!.toDouble();
          x = q5 - q1;
          y = q6 - q2;
          height = sqrt((x * x) + (y * y));
          x = q7 - q5;
  ...
  PdfTextMarkupAnnotationType _getTextMarkupAnnotation(String aType) {
    PdfTextMarkupAnnotationType annotType = PdfTextMarkupAnnotationType.highlight;
    switch (aType) {
      case 'Highlight':
  ...
  /// internal method
  void save() {
    final PdfAnnotationHelper helper = PdfAnnotationHelper.getHelper(textMarkupAnnotation);
    if (PdfAnnotationCollectionHelper.getHelper(textMarkupAnnotation.page!.annotations).flatten) {
      helper.flatten = true;
    }
      ...
      if (helper.flatten) {
        ...
        if (appearance != null) {
          ...
          dictionary!.setProperty(
              PdfDictionaryProperties.ap, PdfReferenceHolder(textMarkupAnnotation.appearance));
        }
      }
    }
  ...
  void _saveTextMarkUpDictionary() {
    dictionary!.setProperty(PdfDictionaryProperties.subtype, PdfName(_getMarkupAnnotationType()));
  }
  ...
  PdfTemplate? _createAppearance() {
    if (!isLoadedAnnotation || (isLoadedAnnotation && setAppearance)) {
      ...
      if (boundsCollection.length > 1) {
        ...
        if (dictionary!.containsKey(PdfDictionaryProperties.quadPoints)) {
          final IPdfPrimitive? mQuadPoints =
              PdfCrossTable.dereference(dictionary![PdfDictionaryProperties.quadPoints]);
            ...
            for (int i = 0; i < (mQuadPoints.count / 8); i++) {
              if (isLoadedAnnotation) {
                ...
                for (int k = 0; k < mQuadPoints.count;) {
                  final double x1 = (mQuadPoints[k]! as PdfNumber).value!.toDouble();
                  final double y1 = (mQuadPoints[k + 1]! as PdfNumber).value!.toDouble();
      ...
      if (boundsCollection.length > 1) {
          ...
          if (textMarkupAnnotationType == PdfTextMarkupAnnotationType.highlight) {
            ...
            graphics.drawLine(
                ...
                Offset(
                    boundsCollection[i].left - rectangle!.x,
                    (boundsCollection[i].top - rectangle.y) +
                        (boundsCollection[i].height - ((boundsCollection[i].height / 2) / 3))),
                Offset(
                    boundsCollection[i].width + (boundsCollection[i].left - rectangle.x),
                    (boundsCollection[i].top - rectangle.y) +
                        (boundsCollection[i].height - ((boundsCollection[i].height / 2) / 3))));
          } else if (textMarkupAnnotationType == PdfTextMarkupAnnotationType.strikethrough) {
            graphics.drawLine(
                PdfPen(color, width: boundsCollection[i].height * 0.05),
                Offset(
                    boundsCollection[i].left - rectangle!.x,
                    (boundsCollection[i].top - rectangle.y) +
                        (boundsCollection[i].height - (boundsCollection[i].height / 2))),
                Offset(
                    boundsCollection[i].width + (boundsCollection[i].left - rectangle.x),
                    (boundsCollection[i].top - rectangle.y) +
                        (boundsCollection[i].height - (boundsCollection[i].height / 2))));
            ...
            graphics.translateTransform(
                boundsCollection[i].left - rectangle!.x, boundsCollection[i].top - rectangle.y);
            graphics.setClip(
                bounds: Rect.fromLTWH(0, 0, boundsCollection[i].width, boundsCollection[i].height));
            graphics.drawPath(_drawSquiggly(boundsCollection[i].width, boundsCollection[i].height),
                pen: pdfPen);
            graphics.restore();
          }
        }
        ...
        if (textMarkupAnnotationType == PdfTextMarkupAnnotationType.highlight) {
          graphics.drawRectangle(
              brush: PdfSolidBrush(color), bounds: Rect.fromLTWH(0, 0, width, height));
        } else if (textMarkupAnnotationType == PdfTextMarkupAnnotationType.underline) {
          graphics.drawLine(PdfPen(color, width: height * 0.05),
              Offset(0, height - ((height / 2) / 3)), Offset(width, height - ((height / 2) / 3)));
        } else if (textMarkupAnnotationType == PdfTextMarkupAnnotationType.strikethrough) {
          graphics.drawLine(PdfPen(color, width: height * 0.05), Offset(0, height / 2),
        ...
        if (isLoadedAnnotation) {
          dictionary![PdfDictionaryProperties.rect] = PdfArray.fromRectangle(rectangle!);
        }
      }
      return template;
    }
  ...
  PdfPath _drawSquiggly(double width, double height) {
    ...
    for (int i = 0; i < mPathPoints.length; i++, x += length) {
      mPathPoints[i] = Offset(x, ((height - location) + zigZag) - (height * 0.02));
      if (zigZag == 0) {
        zigZag = location;
      } else {
        zigZag = 0;
    ...
    if (!isLoadedAnnotation) {
      ...
      if (dictionary != null &&
        ...
        if (appearanceDictionary != null && appearanceDictionary is PdfDictionary) {
          ...
          if (appearanceDictionary != null && appearanceDictionary is PdfStream) {
            appearance = PdfTemplateHelper.fromPdfStream(appearanceDictionary);
            final bool isNormalMatrix = validateTemplateMatrix(appearanceDictionary);
            if (isNormalMatrix && page.rotation != PdfPageRotateAngle.rotateAngle0) {
              flattenAnnotationTemplate(appearance, isNormalMatrix);
            } else if (isNormalMatrix &&
                isValidTemplateMatrix(appearanceDictionary, bounds.topLeft, appearance)) {
              flattenAnnotationTemplate(appearance, isNormalMatrix);
            }
          } else {
            setAppearance = true;
            ...
            if (appearance != null) {
              final bool isNormalMatrix =
                  validateTemplateMatrix(PdfTemplateHelper.getHelper(appearance).content);
              flattenAnnotationTemplate(appearance, isNormalMatrix);
            }
          }
        }
      } else if (!dictionary!.containsKey(PdfDictionaryProperties.ap) && appearance == null) {
        setAppearance = true;
        appearance = _createAppearance();
        if (appearance != null) {
          final bool isNormalMatrix =
