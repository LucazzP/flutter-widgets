...
class PdfEllipseAnnotation extends PdfAnnotation {
  ...
  PdfEllipseAnnotation._(PdfDictionary dictionary, PdfCrossTable crossTable, String text) {
...
class PdfEllipseAnnotationHelper extends PdfAnnotationHelper {
  ...
  PdfEllipseAnnotationHelper(this.annotation, Rect bounds, String text,
    ...
    initializeAnnotation(
        ...
        setAppearance: setAppearance);
    dictionary!
        .setProperty(PdfDictionaryProperties.subtype, PdfName(PdfDictionaryProperties.circle));
  }

  PdfEllipseAnnotationHelper._(this.annotation, PdfDictionary dictionary, PdfCrossTable crossTable)
      : super(annotation) {
    ...
    if (helper.flatten || annotation.setAppearance || pdfAppearance != null) {
      ...
      if (helper.flatten) {
        ...
        if (appearance != null) {
          ...
          helper.dictionary!
              .setProperty(PdfDictionaryProperties.ap, PdfReferenceHolder(annotation.appearance));
        }
      }
    }
    if (!helper.flatten && !isLoadedAnnotation) {
      helper.saveAnnotation();
      helper.dictionary!.setProperty(PdfDictionaryProperties.bs, annotation.border);
    }
  ...
  void _flattenAnnotation(PdfPage? page, PdfTemplate? appearance) {
    ...
    if (helper.isLoadedAnnotation) {
      final bool isContainsAP = helper.dictionary!.containsKey(PdfDictionaryProperties.ap);
      if (appearance == null) {
        if (isContainsAP) {
          PdfDictionary? appearanceDictionary =
              PdfCrossTable.dereference(helper.dictionary![PdfDictionaryProperties.ap])
                  as PdfDictionary?;
      ...
      final Rect rectangle =
          helper.calculateTemplateBounds(annotation.bounds, page, appearance, true);
  ...
  PdfTemplate? _createAppearance() {
    ...
    if (isLoadedAnnotation && helper.dictionary!.containsKey(PdfDictionaryProperties.be)) {
    ...
    final PdfPen mBorderPen = PdfPen(annotation.color, width: annotation.border.width);
    ...
    if (isLoadedAnnotation) {
      ...
      graphics!.drawEllipse(
          Rect.fromLTWH(rect.left + borderWidth, rect.top + borderWidth,
              rect.width - annotation.border.width, rect.height - annotation.border.width),
          pen: paintParams.borderPen,
          brush: paintParams.backBrush);
    }
    if (annotation.opacity < 1) {
  ...
  PdfRectangle _obtainStyle(PdfPen mBorderPen, PdfRectangle rectangle, double borderWidth) {
    final PdfDictionary dictionary = PdfAnnotationHelper.getHelper(annotation).dictionary!;
    if (dictionary.containsKey(PdfDictionaryProperties.bs)) {
      final PdfDictionary? bSDictionary =
          PdfCrossTable.dereference(dictionary[PdfDictionaryProperties.bs]) as PdfDictionary?;
    ...
    if (!PdfAnnotationHelper.getHelper(annotation).isBounds &&
      ...
      final PdfArray? mRdArray =
          PdfCrossTable.dereference(dictionary[PdfDictionaryProperties.rd]) as PdfArray?;
      if (mRdArray != null) {
  ...
  void _drawAppearance(
      PdfRectangle rectangle, double borderWidth, PdfGraphics? graphics, PaintParams paintParams) {
    ...
    if (PdfAnnotationHelper.getHelper(annotation)
      ...
      final PdfArray? rdArray = PdfCrossTable.dereference(PdfAnnotationHelper.getHelper(annotation)
          .dictionary!
    ...
    if (radius! > 0) {
      ...
      for (int i = 0; i < controlPointList.length; i++) {
        _createBezier(startPointList[i], controlPointList[i], endPointList[i], points);
      }
  ...
  // Create bezier curve
  void _createBezier(Offset ctrl1, Offset ctrl2, Offset ctrl3, List<Offset> bezierPoints) {
  ...
  void _populateBezierPoints(
      Offset ctrl1, Offset ctrl2, Offset ctrl3, int currentIteration, List<Offset> bezierPoints) {
    if (currentIteration < 2) {
      //calculate next mid points
      final Offset midPoint1 = Offset((ctrl1.dx + ctrl2.dx) / 2, (ctrl1.dy + ctrl2.dy) / 2);
      final Offset midPoint2 = Offset((ctrl2.dx + ctrl3.dx) / 2, (ctrl2.dy + ctrl3.dy) / 2);
      final Offset midPoint3 =
          Offset((midPoint1.dx + midPoint2.dx) / 2, (midPoint1.dy + midPoint2.dy) / 2);
      //the next control point
      currentIteration++;
      _populateBezierPoints(
          ctrl1, midPoint1, midPoint3, currentIteration, bezierPoints); //left branch
      bezierPoints.add(midPoint3);
      //add the next control point
      _populateBezierPoints(
          midPoint3, midPoint2, ctrl3, currentIteration, bezierPoints); //right branch
    }
  }
