...
class PdfLineAnnotation extends PdfAnnotation {
  ...
  PdfLineAnnotation._(PdfDictionary dictionary, PdfCrossTable crossTable, String annotText) {
  ...
  List<int> get linePoints => PdfAnnotationHelper.getHelper(this).isLoadedAnnotation
  ...
  set linePoints(List<int> value) {
    ...
    if (helper.isLoadedAnnotation) {
      helper.dictionary!.setProperty(PdfDictionaryProperties.l, _helper.linePoints);
    }
  }

  /// Gets the line intent of the annotation.
  PdfLineIntent get lineIntent => PdfAnnotationHelper.getHelper(this).isLoadedAnnotation
  ...
  set lineIntent(PdfLineIntent value) {
    ...
    if (!helper.isLoadedAnnotation) {
      ...
      helper.dictionary!.setName(PdfName(PdfDictionaryProperties.it), helper.getEnumName(value));
    }
  }
  ...
  int get leaderLineExt => PdfAnnotationHelper.getHelper(this).isLoadedAnnotation
  ...
  PdfLineEndingStyle get beginLineStyle => PdfAnnotationHelper.getHelper(this).isLoadedAnnotation
  ...
  set beginLineStyle(PdfLineEndingStyle value) {
    ...
    if (!helper.isLoadedAnnotation) {
      ...
      lineStyle.insert(0,
          PdfName(helper.getEnumName(_helper._getLineStyle(helper.getEnumName(value.toString())))));
      helper.dictionary!.setProperty(PdfDictionaryProperties.le, lineStyle);
    }
  }

  /// Gets the end line style of the annotation.
  PdfLineEndingStyle get endLineStyle => PdfAnnotationHelper.getHelper(this).isLoadedAnnotation
  ...
  PdfLineCaptionType get captionType => PdfAnnotationHelper.getHelper(this).isLoadedAnnotation
  ...
  set captionType(PdfLineCaptionType value) {
    ...
    if (!helper.isLoadedAnnotation) {
      ...
      helper.dictionary!.setProperty(
          ...
          PdfName(
              helper.getEnumName(_helper._getCaptionType(helper.getEnumName(value.toString())))));
    }
  }
  ...
  set text(String value) {
    ...
    if (helper.textValue != value) {
      ...
      helper.dictionary!.setString(PdfDictionaryProperties.contents, helper.textValue);
    }
  }
...
class PdfLineAnnotationHelper extends PdfAnnotationHelper {
  ...
  PdfLineAnnotationHelper(this.lineAnnotation, List<int> linePoints, String text,
      {PdfColor? color,
      PdfColor? innerColor,
      PdfAnnotationBorder? border,
      String? author,
    ...
    points = linePoints;
    dictionary!.setProperty(PdfDictionaryProperties.subtype, PdfName(PdfDictionaryProperties.line));
  }
  PdfLineAnnotationHelper._(this.lineAnnotation, PdfDictionary dictionary, PdfCrossTable crossTable)
      : super(lineAnnotation) {
  ...
  /// internal method
  void save() {
    final PdfAnnotationHelper helper = PdfAnnotationHelper.getHelper(lineAnnotation);
    if (PdfAnnotationCollectionHelper.getHelper(lineAnnotation.page!.annotations).flatten) {
      helper.flatten = true;
    }
    if (helper.flatten || lineAnnotation.setAppearance) {
      ...
      if (helper.flatten) {
        ...
        if (appearance != null) {
          ...
          helper.dictionary!.setProperty(
              PdfDictionaryProperties.ap, PdfReferenceHolder(lineAnnotation.appearance));
        }
      }
    }
  ...
  void _savePdfLineDictionary() {
    ...
    if (lineStyle!.elements.isNotEmpty) {
      lineStyle!.insert(0, PdfName(helper.getEnumName(lineAnnotation.beginLineStyle)));
      lineStyle!.insert(1, PdfName(helper.getEnumName(lineAnnotation.endLineStyle)));
    } else {
      lineStyle!.add(PdfName(helper.getEnumName(lineAnnotation.beginLineStyle)));
    ...
    if (!lineAnnotation.innerColor.isEmpty &&
      ...
      dictionary.setProperty(
          PdfDictionaryProperties.ic, PdfColorHelper.toArray(lineAnnotation.innerColor));
    }
    dictionary[PdfDictionaryProperties.c] = PdfColorHelper.toArray(lineAnnotation.color);

    dictionary.setProperty(
        PdfDictionaryProperties.it, PdfName(helper.getEnumName(lineAnnotation.lineIntent)));

    dictionary.setProperty(PdfDictionaryProperties.lle, PdfNumber(lineAnnotation.leaderLineExt));

    dictionary.setProperty(PdfDictionaryProperties.ll, PdfNumber(leaderLine));

    dictionary.setProperty(
        PdfDictionaryProperties.cp, PdfName(helper.getEnumName(lineAnnotation.captionType)));

    dictionary.setProperty(PdfDictionaryProperties.cap, PdfBoolean(lineAnnotation.lineCaption));

    dictionary.setProperty(
        PdfDictionaryProperties.rect, PdfArray.fromRectangle(_obtainLineBounds()));
  }
  ...
  void _flattenAnnotation(PdfPage? page, PdfTemplate? appearance) {
    ...
    final PdfDictionary dictionary = helper.dictionary!;
    if (helper.isLoadedAnnotation) {
      final bool isContainsAP = dictionary.containsKey(PdfDictionaryProperties.ap);
      if (isContainsAP && appearance == null) {
        PdfDictionary? appearanceDictionary =
            PdfCrossTable.dereference(dictionary[PdfDictionaryProperties.ap]) as PdfDictionary?;
        if (appearanceDictionary != null) {
          appearanceDictionary =
              PdfCrossTable.dereference(appearanceDictionary[PdfDictionaryProperties.n])
                  as PdfDictionary?;
          if (appearanceDictionary != null) {
            final PdfStream appearanceStream = appearanceDictionary as PdfStream;
            appearance = PdfTemplateHelper.fromPdfStream(appearanceStream);
            final bool isNormalMatrix = helper.validateTemplateMatrix(appearanceDictionary);
            ...
            if (appearance != null) {
              final bool isNormalMatrix =
                  helper.validateTemplateMatrix(PdfTemplateHelper.getHelper(appearance).content);
              helper.flattenAnnotationTemplate(appearance, isNormalMatrix);
            }
          }
        }
      ...
      final Rect rectangle =
          helper.calculateTemplateBounds(lineAnnotation.bounds, page, appearance, true);
      if (lineAnnotation.opacity < 1) {
        page.graphics.setTransparency(lineAnnotation.opacity);
      }
      page.graphics
          .drawPdfTemplate(appearance!, Offset(rectangle.left, rectangle.top), rectangle.size);
  ...
  PdfTemplate? _createAppearance() {
    ...
    PdfAnnotationHelper.setMatrixToZeroRotation(PdfTemplateHelper.getHelper(template).content);
    ...
    final PdfPen mBorderPen = PdfPen(lineAnnotation.color, width: lineAnnotation.border.width);
    if (lineAnnotation.border.borderStyle == PdfBorderStyle.dashed) {
    ...
    final PdfFont mFont = PdfStandardFont(PdfFontFamily.helvetica, isLoadedAnnotation ? 10 : 9,
        style: PdfFontStyle.regular);
    final PdfStringFormat format = PdfStringFormat();
    format.alignment = PdfTextAlignment.center;
    format.lineAlignment = PdfVerticalAlignment.middle;
    final double lineWidth = mFont.measureString(lineAnnotation.text, format: format).width;
    ...
    if (linePoints.length == 4) {
      ...
      final double line = leadLine + (isLoadedAnnotation ? _obtainLeaderOffset().toDouble() : 0);
      final List<double> startingPoint = helper.getAxisValue(x1y1, lineAngle + 90, line);
      final List<double> endingPoint = helper.getAxisValue(x2y2, lineAngle + 90, line);
      final double lineDistance = sqrt(
          pow(endingPoint[0] - startingPoint[0], 2) + pow(endingPoint[1] - startingPoint[1], 2));
      final double centerWidth = lineDistance / 2 - ((lineWidth / 2) + lineAnnotation.border.width);
      final List<double> middlePoint1 = helper.getAxisValue(startingPoint, angle, centerWidth);
      final List<double> middlePoint2 = helper.getAxisValue(endingPoint, angle + 180, centerWidth);
      ...
      if (lineAnnotation.beginLineStyle == PdfLineEndingStyle.openArrow ||
        ...
        lineStartingPoint = helper.getAxisValue(startingPoint, angle, lineAnnotation.border.width);
      } else {
      ...
      if (lineAnnotation.endLineStyle == PdfLineEndingStyle.openArrow ||
        ...
        lineEndingPoint = helper.getAxisValue(endingPoint, angle, -lineAnnotation.border.width);
      } else {
      ...
      if (lineAnnotation.text.isEmpty || caption == 'Top' || !lineAnnotation.lineCaption) {
        graphics!.drawLine(mBorderPen, Offset(lineStartingPoint[0], -lineStartingPoint[1]),
            Offset(lineEndingPoint[0], -lineEndingPoint[1]));
      } else {
        graphics!.drawLine(mBorderPen, Offset(lineStartingPoint[0], -lineStartingPoint[1]),
            Offset(middlePoint1[0], -middlePoint1[1]));
        graphics.drawLine(mBorderPen, Offset(lineEndingPoint[0], -lineEndingPoint[1]),
      ...
      helper.setLineEndingStyles(startingPoint, endingPoint, graphics, angle, mBorderPen, backBrush,
          lineStyle, borderLength);
      //Set leader extension.
      final List<double> beginLineExt = helper.getAxisValue(
          startingPoint, lineAngle + 90, lineAnnotation.leaderLineExt.toDouble());
      ...
      final List<double> endLineExt =
          helper.getAxisValue(endingPoint, lineAngle + 90, lineAnnotation.leaderLineExt.toDouble());
      ...
      final List<double> endLeaderLine = helper.getAxisValue(endingPoint, lineAngle - 90, leadLine);
      graphics.drawLine(mBorderPen, Offset(endingPoint[0], -endingPoint[1]),
          Offset(endLeaderLine[0], -endLeaderLine[1]));
      //Set caption Type.
      if (lineAnnotation.lineCaption) {
        final double midpoint = lineDistance / 2;
        final List<double> centerPoint = helper.getAxisValue(startingPoint, angle, midpoint);
    ...
    if (isLoadedAnnotation) {
      helper.dictionary!
          .setProperty(PdfDictionaryProperties.rect, PdfArray.fromRectangle(_obtainLineBounds()));
    }
    if (!isLoadedAnnotation && helper.flatten) {
      ...
      if (lineAnnotation.page != null) {
        lineAnnotation.bounds = Rect.fromLTWH(
            ...
            pageHeight - (nativeRectangle.top + nativeRectangle.height) - margins.top,
  ...
  List<double> _getCaptionPosition(
    ...
    final PdfAnnotationHelper helper = PdfAnnotationHelper.getHelper(lineAnnotation);
    if (helper.isLoadedAnnotation) {
      final bool isContainsMeasure =
          helper.dictionary!.items!.containsKey(PdfName(PdfDictionaryProperties.measure));
      ...
      captionPosition = helper.getAxisValue(
          centerPoint, angle + 90, caption == 'Top' ? font.height : font.height / 2);
    }
  ...
  int _obtainLeaderLine() {
    ...
    if (helper.dictionary!.containsKey(PdfDictionaryProperties.ll)) {
      final PdfNumber ll = helper.dictionary![PdfDictionaryProperties.ll]! as PdfNumber;
  ...
  PdfLineIntent _obtainLineIntent() {
    ...
    if (helper.dictionary!.containsKey(PdfDictionaryProperties.it)) {
      final PdfName lineintent =
          helper.crossTable.getObject(helper.dictionary![PdfDictionaryProperties.it])! as PdfName;
  ...
  List<int> _obtainLinePoints() {
    ...
    if (!PdfAnnotationHelper.getHelper(lineAnnotation).isLoadedAnnotation) {
      ...
      if (PdfAnnotationHelper.getHelper(lineAnnotation)
        ...
        linePoints = PdfCrossTable.dereference(PdfAnnotationHelper.getHelper(lineAnnotation)
  ...
  PdfRectangle _obtainLineBounds() {
    ...
    if (points.length == 4 || PdfAnnotationHelper.getHelper(lineAnnotation).isLoadedAnnotation) {
      ...
      if (lPoints.length == 4) {
        ...
        bounds = PdfAnnotationHelper.getHelper(lineAnnotation).isLoadedAnnotation
            ...
            : PdfAnnotationHelper.getHelper(lineAnnotation).calculateLineBounds(
                ...
                lineAnnotation.border.width);
        bounds = PdfRectangle(
            bounds.left - 8, bounds.top - 8, bounds.width + 2 * 8, bounds.height + 2 * 8);
      }
    }
    return bounds;
  }
  ...
  PdfArray? _obtainLineStyle() {
    ...
    if (helper.dictionary!.containsKey(PdfDictionaryProperties.le)) {
      array =
          helper.crossTable.getObject(helper.dictionary![PdfDictionaryProperties.le]) as PdfArray?;
    }
    return array;
  }
  ...
  /// internal method
  static PdfLineAnnotation load(PdfDictionary dictionary, PdfCrossTable crossTable, String text) {
