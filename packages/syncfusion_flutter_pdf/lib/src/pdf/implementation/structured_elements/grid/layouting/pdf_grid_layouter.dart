...
class PdfGridLayouter extends ElementLayouter {
  ...
  void layoutGrid(PdfGraphics graphics, PdfRectangle bounds) {
    ...
    if (PdfGraphicsHelper.getHelper(_currentGraphics!).layer != null &&
      ...
      final int index = PdfSectionHelper.getHelper(
              PdfPageHelper.getHelper(PdfGraphicsHelper.getHelper(_currentGraphics!).page!)
                  .section!)
          .indexOf(PdfGraphicsHelper.getHelper(_currentGraphics!).page!);
      if (!PdfGridHelper.getHelper(_grid!).listOfNavigatePages.contains(index)) {
  ...
  PdfLayoutFormat _getFormat(PdfLayoutFormat? format) {
    return format != null ? PdfLayoutFormat.fromFormat(format) : PdfLayoutFormat();
  }
  ...
  void _determineColumnDrawRanges() {
    ...
    int endColumn = 0;
    double cellWidths = 0;
    final double availableWidth = _currentGraphics!.clientSize.width - _currentBounds.x;
  ...
  PdfLayoutResult? _layoutOnPage(PdfLayoutParams param) {
    ...
    for (int rangeIndex = 0; rangeIndex < _columnRanges!.length; rangeIndex++) {
      ...
      if (_currentPage != null) {
        final Map<String, dynamic> pageLayoutResult =
            _raiseBeforePageLayout(_currentPage, _currentBounds.rect, _currentRowIndex);
        _currentBounds = PdfRectangle.fromRect(pageLayoutResult['currentBounds']);
        _currentRowIndex = pageLayoutResult['currentRow'] as int;
        if (pageLayoutResult['cancel'] as bool) {
          result = PdfLayoutResultHelper.load(_currentPage!, _currentBounds.rect);
          break;
        }
      }
      bool drawHeader;
      if (PdfGridHelper.getHelper(_grid!).isBuiltinStyle &&
        ...
        if (PdfGridHelper.getHelper(_grid!).gridBuiltinStyle != PdfGridBuiltInStyle.tableGrid) {
          PdfGridHelper.getHelper(_grid!)
              .applyBuiltinStyles(PdfGridHelper.getHelper(_grid!).gridBuiltinStyle);
        }
      }
        ...
        if (startPage != _currentPage) {
          for (int k = _cellStartIndex; k <= _cellEndIndex; k++) {
            if (PdfGridCellHelper.getHelper(row.cells[k]).isCellMergeContinue) {
              PdfGridCellHelper.getHelper(row.cells[k]).isCellMergeContinue = false;
              row.cells[k].value = '';
            }
          }
        }
      ...
      bool flag = true;
      if (isParentCell!) {
        _cellEndIndex = _cellStartIndex = PdfGridHelper.getHelper(_grid!).parentCellIndex;
        _parentCellIndexList = <int>[];
        _parentCellIndexList.add(PdfGridHelper.getHelper(_grid!).parentCellIndex);
        PdfGridCellHelper.getHelper(PdfGridHelper.getHelper(_grid!).parentCell!).present = true;
        ...
        while (PdfGridHelper.getHelper(parentGrid).parentCell != null) {
          _parentCellIndexList.add(PdfGridHelper.getHelper(parentGrid).parentCellIndex);
          ...
          PdfGridCellHelper.getHelper(PdfGridHelper.getHelper(parentGrid).parentCell!).present =
              true;
          parentGrid = PdfGridRowHelper.getHelper(
                  PdfGridCellHelper.getHelper(PdfGridHelper.getHelper(parentGrid).parentCell!).row!)
              .grid;
        ...
        if ((!PdfGridHelper.getHelper(parentGrid).isDrawn) ||
            (!PdfGridHelper.getHelper(parentGrid).listOfNavigatePages.contains(index))) {
          section = PdfPageHelper.getHelper(PdfGraphicsHelper.getHelper(_currentGraphics!).page!)
              .section!;
          ...
          for (int rowIndex = 0; rowIndex < parentGrid.rows.count; rowIndex++) {
            ...
            PdfGridCellHelper.getHelper(cell)
                .draw(_currentGraphics, PdfRectangle(location.x, location.y, width, height), false);
      ...
      for (int rowIndex = 0; rowIndex < _grid!.rows.count; rowIndex++) {
        ...
        repeatRowIndex = -1;
        if (flag && PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).isChildGrid!) {
          startingHeight = originalHeight;
          flag = false;
        }
        if (PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).isChildGrid! &&
            PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell!.rowSpan > 1 &&
            (startingHeight! + _childHeight).toInt() < (_currentBounds.y + row.height).toInt()) {
          if (_grid!.rows.count > i) {
            final PdfGrid temp = PdfGridRowHelper.getHelper(PdfGridCellHelper.getHelper(
                        PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell!)
                    .row!)
                .grid;
            for (int tempRowIndex = 0; tempRowIndex < temp.rows.count; tempRowIndex++) {
              final PdfGridRow tempRow = temp.rows[tempRowIndex];
              if (tempRow.cells[PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid)
                      .parentCellIndex] ==
                  PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell) {
                final dynamic grid = tempRow
                    .cells[PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid)
                        .parentCellIndex]
                    .value;
                if (grid is PdfGrid) {
                  PdfGridRowCollectionHelper.getRows(grid.rows).removeRange(0, i - 1);
                }
              }
            }
          }
        ...
        if (PdfGridRowHelper.getHelper(row).isRowBreaksNextPage) {
          ...
          for (int l = 0; l < row.cells.count; l++) {
            ...
            if (row.height == row.cells[l].height && row.cells[l].value is PdfGrid) {
              final PdfGrid grid = row.cells[l].value as PdfGrid;
              for (int m = grid.rows.count; 0 < m; m--) {
                if (PdfGridRowHelper.getHelper(grid.rows[m - 1]).rowBreakHeight > 0) {
                  isNestedRowBreak = true;
                  break;
                }
                if (PdfGridRowHelper.getHelper(grid.rows[m - 1]).isRowBreaksNextPage) {
                  PdfGridRowHelper.getHelper(row).rowBreakHeight =
                      PdfGridRowHelper.getHelper(grid.rows[m - 1]).rowBreakHeight;
                  break;
                }
                PdfGridRowHelper.getHelper(row).rowBreakHeight =
                    PdfGridRowHelper.getHelper(row).rowBreakHeight + grid.rows[m - 1].height;
              }
            }
            if (isNestedRowBreak) {
              break;
          ...
          for (int j = 0; j < row.cells.count; j++) {
            if (row.height > row.cells[j].height) {
              ...
              final int index = PdfSectionHelper.getHelper(section).indexOf(page);
              for (int k = 0;
                  k < (PdfSectionHelper.getHelper(section).pageReferences!.count - 1) - index;
                  k++) {
                rect = PdfRectangle(x, 0, PdfGridRowHelper.getHelper(row).grid.columns[j].width,
                    page.getClientSize().height);
                repeatRowIndex = -1;
                PdfGridCellHelper.getHelper(row.cells[j]).draw(page.graphics, rect, false);
        ...
        while (!rowResult.isFinish && startPage != null) {
          ...
          if (startPage != _currentPage) {
            if (PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).isChildGrid! &&
                PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell != null) {
              final PdfRectangle bounds = PdfRectangle(format.paginateBounds.left,
                  format.paginateBounds.top, param.bounds!.width, tempResult.bounds.height);
              ...
              for (int c = 0; c < row.cells.count; c++) {
                ...
                if (cell.columnSpan > 1) {
                  for (; c < cell.columnSpan; c++) {
                    cellWidth += PdfGridRowHelper.getHelper(row).grid.columns[c].width;
                  }
                } else {
                  cellWidth =
                      max(cell.width, PdfGridRowHelper.getHelper(row).grid.columns[c].width);
                }
                _currentGraphics = PdfGridCellHelper.getHelper(cell).drawCellBorders(
                    _currentGraphics!, PdfRectangle(bounds.x, bounds.y, cellWidth, bounds.height));
          ...
          if (repeatRow) {
            ...
            if (PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell != null) {
              PdfGridRowHelper.getHelper(PdfGridCellHelper.getHelper(
                          PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell!)
                      .row!)
              ...
              PdfGridRowHelper.getHelper(PdfGridCellHelper.getHelper(
                          PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell!)
                      .row!)
                  .rowBreakHeight = PdfGridRowHelper.getHelper(row)
                      .rowBreakHeight +
            ...
            if (PdfGridRowHelper.getHelper(row).noOfPageCount > 1) {
              final double temp = PdfGridRowHelper.getHelper(row).rowBreakHeight;
              for (int j = 1; j < PdfGridRowHelper.getHelper(row).noOfPageCount; j++) {
                PdfGridRowHelper.getHelper(row).rowBreakHeight = 0;
                row.height = (PdfGridRowHelper.getHelper(row).noOfPageCount - 1) *
        ...
        if (!rowResult.isFinish &&
          ...
          if (!PdfGridHelper.getHelper(_grid!).isSingleGrid) {
            for (int j = 0; j < _grid!.rows.count; j++) {
              ...
              if (isWidthGreaterthanParent &&
                  PdfGridCellHelper.getHelper(_grid!.rows[j].cells[_rowBreakPageHeightCellIndex])
                          .pageCount >
                      0) {
                isAddNextPage = true;
              }
          ...
          if (!PdfGridHelper.getHelper(_grid!).isRearranged && isAddNextPage) {
            ...
            _currentBounds = PdfRectangle(0, 0, clientSize.width, clientSize.height);
            final int pageindex = PdfSectionHelper.getHelper(
                    PdfPageHelper.getHelper(PdfGraphicsHelper.getHelper(_currentGraphics!).page!)
                        .section!)
                .indexOf(PdfGraphicsHelper.getHelper(_currentGraphics!).page!);
            if (!PdfGridHelper.getHelper(_grid!).listOfNavigatePages.contains(pageindex)) {
            ...
            if (endArgs.nextPage == null) {
              ...
              _currentBounds = PdfRectangle(
                  0, 0, _currentGraphics!.clientSize.width, _currentGraphics!.clientSize.height);
            }
          }
          final bool isSameSection = PdfPageHelper.getHelper(_currentPage!).section ==
          ...
          if (_currentBounds.x == PdfGridHelper.getHelper(_grid!).defaultBorder.left.width / 2) {
            _currentBounds.y = _currentBounds.y + _startLocation.x;
          }
          if (_grid!.repeatHeader) {
            for (int headerIndex = 0; headerIndex < _grid!.headers.count; headerIndex++) {
              _drawRow(_grid!.headers[headerIndex]);
            }
          }
          _drawRow(row);
          if (_currentPage != null && !layoutedPages.containsKey(_currentPage)) {
        ...
        if (PdfGridRowHelper.getHelper(row).gridResult != null) {
          ...
          _currentGraphics = _currentPage!.graphics;
          _startLocation = PdfPoint(PdfGridRowHelper.getHelper(row).gridResult!.bounds.left,
              PdfGridRowHelper.getHelper(row).gridResult!.bounds.top);
          _currentBounds.y = PdfGridRowHelper.getHelper(row).gridResult!.bounds.bottom;
          if (startPage != _currentPage) {
            final PdfSection secion = PdfPageHelper.getHelper(_currentPage!).section!;
            final int startIndex = PdfSectionHelper.getHelper(secion).indexOf(startPage!) + 1;
            final int endIndex = PdfSectionHelper.getHelper(secion).indexOf(_currentPage!);
            for (int page = startIndex; page < endIndex + 1; page++) {
              PdfGraphics pageGraphics =
                  PdfSectionHelper.getHelper(secion).getPageByIndex(page)!.graphics;
              final PdfPoint location =
                  PdfPoint(format.paginateBounds.left, format.paginateBounds.top);
              if (location == PdfPoint.empty &&
                  _currentBounds.x > location.x &&
                  !PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).isChildGrid! &&
                  PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).parentCell ==
                      null) {
                location.x = _currentBounds.x;
              }
              double height = page == endIndex
                  ? (PdfGridRowHelper.getHelper(row).gridResult!.bounds.height - param.bounds!.y)
              ...
              for (int c = 0; c < row.cells.count; c++) {
                ...
                if (cell.columnSpan > 1) {
                  ...
                  cellWidth = PdfGridHelper.getHelper(_grid!).isWidthSet
                      ? min(cell.width, PdfGridRowHelper.getHelper(row).grid.columns[c].width)
                      : max(cell.width, PdfGridRowHelper.getHelper(row).grid.columns[c].width);
                }
                pageGraphics = PdfGridCellHelper.getHelper(cell).drawCellBorders(
                    pageGraphics, PdfRectangle(location.x, location.y, cellWidth, height));
      ...
      for (int c = 0; c < _grid!.rows.count; c++) {
        if (_cellEndIndex != -1 && _grid!.rows[c].cells[_cellEndIndex].value is PdfGrid) {
          final PdfGrid grid = _grid!.rows[c].cells[_cellEndIndex].value as PdfGrid;
          PdfGridHelper.getHelper(_grid!).rowLayoutBoundswidth =
              PdfGridHelper.getHelper(grid).rowLayoutBoundswidth;
          isPdfGrid = true;
          if (largeNavigatePage[0][0]! < PdfGridHelper.getHelper(grid).listOfNavigatePages.length) {
            largeNavigatePage[0][0] =
                PdfGridHelper.getHelper(grid).listOfNavigatePages.length.toDouble();
      ...
      if (!isPdfGrid && cellBounds.isNotEmpty) {
        ...
        PdfGridHelper.getHelper(_grid!).rowLayoutBoundswidth = maximumCellBoundsWidth;
      } else {
        PdfGridHelper.getHelper(_grid!).rowLayoutBoundswidth = largeNavigatePage[0][1]!;
      }
      if (_columnRanges!.indexOf(range) < _columnRanges!.length - 1 &&
        ...
        if (largeNavigatePage[0][0]!.toInt() != 0) {
          final PdfSection section = PdfPageHelper.getHelper(_currentPage!).section!;
          final int pageIndex = PdfSectionHelper.getHelper(section).indexOf(_currentPage!);
    ...
    if (_currentPage != null) {
      ...
      if (_grid!.style.allowHorizontalOverflow &&
          _grid!.style.horizontalOverflowType == PdfHorizontalOverflowType.nextPage) {
  ...
  bool _drawParentGridRow(PdfGrid grid) {
    ...
    for (int j = 0; j < grid.rows.count; j++) {
      if (PdfGridCellHelper.getHelper(grid.rows[j].cells[_cellStartIndex]).present) {
        present = true;
        if (grid.rows[j].cells[_cellStartIndex].value is PdfGrid) {
          final PdfGrid? childGrid = grid.rows[j].cells[_cellStartIndex].value as PdfGrid?;
          PdfGridCellHelper.getHelper(grid.rows[j].cells[_cellStartIndex]).present = false;
          if (childGrid == _grid) {
            if (!_isChildGrid!) {
              _currentBounds.y = y!;
            } else {
              if (j == 0) {
                _currentBounds.y = _currentBounds.y - PdfGridHelper.getHelper(grid).size.height;
              } else {
                int k = j;
            ...
            _currentBounds.x =
                _currentBounds.x + grid.style.cellPadding.left + grid.style.cellPadding.right;
            _currentBounds.y =
                _currentBounds.y + grid.style.cellPadding.top + grid.style.cellPadding.bottom;
            _currentBounds.width = _currentBounds.width - 2 * _currentBounds.x;
            break;
          } else {
            _isChildGrid = true;
            if (_parentCellIndexList.isNotEmpty) {
              _cellStartIndex = _parentCellIndexList[_parentCellIndexList.length - 1];
              _parentCellIndexList.removeAt(_parentCellIndexList.length - 1);
            }
            _currentBounds.y = y!;
            _currentBounds.x =
            ...
            if (!isPresent) {
              _currentBounds.y = _currentBounds.y - PdfGridHelper.getHelper(childGrid).size.height;
            }
            _isChildGrid = false;
            break;
          }
  ...
  _RowLayoutResult? _drawRow(PdfGridRow? row, [_RowLayoutResult? result, double? height]) {
    if (result == null && height == null) {
      ...
      if (PdfGridRowHelper.getHelper(row!).rowSpanExists) {
        ...
        if (currRowIndex == -1) {
          currRowIndex = PdfGridHeaderCollectionHelper.getHelper(_grid!.headers).indexOf(row);
          if (currRowIndex != -1) {
            isHeader = true;
          }
        }
        for (int i = currRowIndex; i < currRowIndex + maxSpan; i++) {
          rowHeightWithSpan += isHeader ? _grid!.headers[i].height : _grid!.rows[i].height;
        }
        if ((rowHeightWithSpan > _currentBounds.height ||
          ...
          for (int cellIndex = 0; cellIndex < row.cells.count; cellIndex++) {
            ...
            for (int i = currRowIndex; i < currRowIndex + maxSpan; i++) {
              rowHeightWithSpan += isHeader ? _grid!.headers[i].height : _grid!.rows[i].height;
              if ((_currentBounds.y + rowHeightWithSpan) > _currentPageBounds.height) {
                rowHeightWithSpan -= isHeader ? _grid!.headers[i].height : _grid!.rows[i].height;
                for (int j = 0; j < _grid!.rows[currRowIndex].cells.count; j++) {
                  final int newSpan = i - currRowIndex;
                  if (!isHeader && (_grid!.rows[currRowIndex].cells[j].rowSpan == maxSpan)) {
                    _grid!.rows[currRowIndex].cells[j].rowSpan = newSpan == 0 ? 1 : newSpan;
                    PdfGridRowHelper.getHelper(_grid!.rows[currRowIndex]).maximumRowSpan =
                        newSpan == 0 ? 1 : newSpan;
                    _grid!.rows[i].cells[j].rowSpan = maxSpan - newSpan;
                    PdfGrid? pdfGrid;
                    if (_grid!.rows[currRowIndex].cells[j].value is PdfGrid) {
                      pdfGrid = _grid!.rows[currRowIndex].cells[j].value as PdfGrid?;
                    }
                    _grid!.rows[i].cells[j].stringFormat =
                        _grid!.rows[currRowIndex].cells[j].stringFormat;
                    _grid!.rows[i].cells[j].style = _grid!.rows[currRowIndex].cells[j].style;
                    ...
                    if (pdfGrid is PdfGrid &&
                        _currentBounds.y +
                            ...
                            _currentBounds.height) {
                      _grid!.rows[i].cells[j].value = _grid!.rows[currRowIndex].cells[j].value;
                    } else if (pdfGrid is! PdfGrid) {
                      _grid!.rows[i].cells[j].value = _grid!.rows[currRowIndex].cells[j].value;
                    }
                    if (i > 0) {
                      PdfGridRowHelper.getHelper(_grid!.rows[i - 1]).rowSpanExists = true;
                    }
                    PdfGridCellHelper.getHelper(_grid!.rows[i].cells[j]).isRowMergeContinue = false;
                  } else if (isHeader &&
                      (_grid!.headers[currRowIndex].cells[j].rowSpan == maxSpan)) {
                    _grid!.headers[currRowIndex].cells[j].rowSpan = newSpan == 0 ? 1 : newSpan;
                    ...
                    _grid!.headers[i].cells[j].style = _grid!.headers[currRowIndex].cells[j].style;
                    _grid!.headers[i].cells[j].columnSpan =
                        _grid!.headers[currRowIndex].cells[j].columnSpan;
                    _grid!.headers[i].cells[j].value = _grid!.headers[currRowIndex].cells[j].value;
                    PdfGridRowHelper.getHelper(_grid!.headers[i - 1]).rowSpanExists = false;
                    PdfGridCellHelper.getHelper(_grid!.headers[i].cells[j]).isRowMergeContinue =
                        false;
                  }
                }
                break;
        ...
        if (height +
          ...
          if (_grid!.allowRowBreakingAcrossPages) {
            ...
            _currentBounds.y +
                    ...
                    rowHeightWithSpan >
                _currentPageBounds.height) {
          if (repeatRowIndex > -1 && repeatRowIndex == PdfGridRowHelper.getHelper(row).index) {
            if (_grid!.allowRowBreakingAcrossPages) {
              result.isFinish = true;
              if (PdfGridHelper.getHelper(_grid!).isChildGrid! &&
                  PdfGridRowHelper.getHelper(row).rowBreakHeight > 0) {
      ...
      if (PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row!).grid).isChildGrid! &&
          ...
          _startLocation.x != _currentBounds.x &&
          PdfGridRowHelper.getHelper(row).getWidth() < _currentPage!.getClientSize().width) {
      ...
      for (int i = _cellStartIndex; i <= _cellEndIndex; i++) {
        final bool cancelSpans = i > _cellEndIndex + 1 && row.cells[i].columnSpan > 1;
        if (!cancelSpans) {
          for (int j = 1; j < row.cells[i].columnSpan; j++) {
            PdfGridCellHelper.getHelper(row.cells[i + j]).isCellMergeContinue = true;
          }
        }
        ...
        if (PdfGridHelper.getHelper(_grid!).isChildGrid! && _grid!.style.allowHorizontalOverflow) {
          if (size.width >= _currentGraphics!.clientSize.width) {
        ...
        final Map<String, dynamic> bclResult = _raiseBeforeCellLayout(
            _currentGraphics,
            PdfGridRowHelper.getHelper(row).isHeaderRow ? _currentHeaderRowIndex : _currentRowIndex,
            i,
        ...
        if (!skipcell!) {
          ...
          final PdfStringLayoutResult? stringResult = PdfGridCellHelper.getHelper(row.cells[i])
              .draw(_currentGraphics, PdfRectangle(location.x, location.y, size.width, size.height),
                  cancelSpans);
          if (PdfGridRowHelper.getHelper(row).grid.style.allowHorizontalOverflow &&
          ...
          if (PdfGridRowHelper.getHelper(row).grid.style.allowHorizontalOverflow &&
            ...
            row.cells[PdfGridRowHelper.getHelper(row).rowOverflowIndex + 1].value =
                stringResult != null && stringResult.remainder != null
                    ? stringResult.remainder
                    : '';
            row.cells[PdfGridRowHelper.getHelper(row).rowOverflowIndex + 1].stringFormat =
                row.cells[i].stringFormat;
            row.cells[PdfGridRowHelper.getHelper(row).rowOverflowIndex + 1].style =
                row.cells[i].style;
            row.cells[PdfGridRowHelper.getHelper(row).rowOverflowIndex + 1].columnSpan =
        ...
        if (row.cells[i].value is PdfGrid) {
          ...
          for (int k = 0; k < PdfGridHelper.getHelper(grid).listOfNavigatePages.length; k++) {
            final int pageIndex = PdfGridHelper.getHelper(grid).listOfNavigatePages[k];
            if (!PdfGridHelper.getHelper(_grid!).listOfNavigatePages.contains(pageIndex)) {
              PdfGridHelper.getHelper(_grid!).listOfNavigatePages.add(pageIndex);
            }
          }
      ...
      result.bounds = PdfRectangle(result.bounds.x, result.bounds.y, location.x, location.y);
      return null;
    }
  }
  ...
  double _reCalculateHeight(PdfGridRow? row, double height) {
    ...
    for (int i = _cellStartIndex; i <= _cellEndIndex; i++) {
      if (PdfGridCellHelper.getHelper(row!.cells[i]).remainingString != null &&
          PdfGridCellHelper.getHelper(row.cells[i]).remainingString!.isNotEmpty) {
        newHeight = max(newHeight, PdfGridCellHelper.getHelper(row.cells[i]).measureHeight());
      }
    }
    return max(height, newHeight);
  }

  _RowLayoutResult _drawRowWithBreak(PdfGridRow row, _RowLayoutResult result, double? height) {
    final PdfPoint location = _currentBounds.location;
    if (PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).isChildGrid! &&
        PdfGridRowHelper.getHelper(row).grid.allowRowBreakingAcrossPages &&
        _startLocation.x != _currentBounds.x) {
    ...
    for (int cellIndex = 0; cellIndex < row.cells.count; cellIndex++) {
      ...
      if (PdfGridCellHelper.getHelper(cell).measureHeight() == height) {
        PdfGridRowHelper.getHelper(row).rowBreakHeight = cell.value is PdfGrid
            ? 0
            : _currentBounds.y + height - _currentBounds.height < _currentPageBounds.height
                ? _currentBounds.height
                : _currentPageBounds.height;
      }
    }
    for (int i = _cellStartIndex; i <= _cellEndIndex; i++) {
      final bool cancelSpans =
          row.cells[i].columnSpan + i > _cellEndIndex + 1 && row.cells[i].columnSpan > 1;
      if (!cancelSpans) {
        for (int j = 1; j < row.cells[i].columnSpan; j++) {
          PdfGridCellHelper.getHelper(row.cells[i + j]).isCellMergeContinue = true;
        }
      ...
      if (!skipcell) {
        stringResult = PdfGridCellHelper.getHelper(row.cells[i]).draw(_currentGraphics,
            PdfRectangle(location.x, location.y, size.width, size.height), cancelSpans);
      }
      if (PdfGridRowHelper.getHelper(row).rowBreakHeight > 0.0) {
        if (stringResult != null) {
          PdfGridCellHelper.getHelper(row.cells[i]).finished = false;
          PdfGridCellHelper.getHelper(row.cells[i]).remainingString = stringResult.remainder ?? '';
          if (PdfGridHelper.getHelper(PdfGridRowHelper.getHelper(row).grid).isChildGrid!) {
            PdfGridRowHelper.getHelper(row).rowBreakHeight = height - stringResult.size.height;
          }
      ...
      result.isFinish =
          (!result.isFinish) ? result.isFinish : PdfGridCellHelper.getHelper(row.cells[i]).finished;
      if (!cancelSpans) {
        _raiseAfterCellLayout(
            _currentGraphics,
            _currentRowIndex,
      ...
      if (row.cells[i].value is PdfGrid) {
        ...
        PdfGridCellHelper.getHelper(row.cells[i]).pageCount =
            PdfGridHelper.getHelper(grid).listOfNavigatePages.length;
        for (int i = 0; i < PdfGridHelper.getHelper(grid).listOfNavigatePages.length; i++) {
          final int pageIndex = PdfGridHelper.getHelper(grid).listOfNavigatePages[i];
          if (!PdfGridHelper.getHelper(_grid!).listOfNavigatePages.contains(pageIndex)) {
            PdfGridHelper.getHelper(_grid!).listOfNavigatePages.add(pageIndex);
          }
        }
    ...
    _currentBounds.y = _currentBounds.y + (_newheight > 0.0 ? _newheight : height);
    result.bounds = PdfRectangle(result.bounds.x, result.bounds.y, location.x, location.y);
    return result;
  }
  ...
  void _reArrangeLayoutedPages(Map<PdfPage?, List<int>> layoutedPages) {
    final PdfDocument? document = PdfPageHelper.getHelper(_currentPage!).document;
  ...
  PdfPage? _getNextPage(PdfLayoutFormat format) {
    final PdfSection section = PdfPageHelper.getHelper(_currentPage!).section!;
    PdfPage? nextPage;
    final int index = PdfSectionHelper.getHelper(section).indexOf(_currentPage!);
    ...
    final int pageindex = PdfSectionHelper.getHelper(
            PdfPageHelper.getHelper(PdfGraphicsHelper.getHelper(_currentGraphics!).page!).section!)
        .indexOf(PdfGraphicsHelper.getHelper(_currentGraphics!).page!);
    if (!PdfGridHelper.getHelper(_grid!).listOfNavigatePages.contains(pageindex)) {
      PdfGridHelper.getHelper(_grid!).listOfNavigatePages.add(pageindex);
    }
    _currentBounds =
        PdfRectangle(0, 0, _currentGraphics!.clientSize.width, _currentGraphics!.clientSize.height);
    if (PdfRectangle.fromRect(format.paginateBounds) != PdfRectangle.empty) {
  ...
  PdfLayoutResult _getLayoutResult() {
    if (PdfGridHelper.getHelper(_grid!).isChildGrid! && _grid!.allowRowBreakingAcrossPages) {
      for (int rowIndex = 0; rowIndex < _grid!.rows.count; rowIndex++) {
        final PdfGridRow row = _grid!.rows[rowIndex];
    ...
    final Rect bounds = _isChanged
        ? Rect.fromLTWH(_currentLocation.x, _currentLocation.y, _currentBounds.width,
            _currentBounds.y - _currentLocation.y)
        : Rect.fromLTWH(_startLocation.x, _startLocation.y, _currentBounds.width,
  ...
  Map<String, dynamic> _raiseBeforePageLayout(
    ...
    if (PdfLayoutElementHelper.getHelper(element!).raiseBeginPageLayout) {
      final PdfGridBeginPageLayoutArgs args =
          PdfGridBeginPageLayoutArgsHelper.load(currentBounds, currentPage!, currentRow);
      PdfLayoutElementHelper.getHelper(element!).onBeginPageLayout(args);
      if (PdfRectangle.fromRect(currentBounds) != PdfRectangle.fromRect(args.bounds)) {
        _isChanged = true;
        _currentLocation = PdfPoint(args.bounds.left, args.bounds.top);
        PdfGridHelper.getHelper(_grid!).measureColumnsWidth(PdfRectangle(args.bounds.left,
            args.bounds.top, args.bounds.width + args.bounds.left, args.bounds.height));
      }
      cancel = args.cancel;
  ...
  PdfGridEndPageLayoutArgs _raisePageLayouted(PdfLayoutResult result) {
    final PdfGridEndPageLayoutArgs args = PdfGridEndPageLayoutArgsHelper.load(result);
  ...
  Map<String, dynamic> _raiseBeforeCellLayout(PdfGraphics? graphics, int rowIndex, int cellIndex,
      PdfRectangle bounds, String value, PdfGridCellStyle? style, bool isHeaderRow) {
  ...
  void _raiseAfterCellLayout(PdfGraphics? graphics, int rowIndex, int cellIndex,
      PdfRectangle bounds, String value, PdfGridCellStyle? cellstyle, bool isHeaderRow) {
  ...
  PdfLayoutResult? layoutInternal(PdfLayoutParams param) {
    ...
    if (_currentPage != null) {
      ...
      if (pageHeight > pageWidth ||
          (PdfPageHelper.getHelper(param.page!).orientation == PdfPageOrientation.landscape &&
          ...
          !PdfPageHelper.getHelper(PdfGraphicsHelper.getHelper(_currentGraphics!).page!)
              ...
              ? PdfSectionHelper.getHelper(
                      PdfPageHelper.getHelper(PdfGraphicsHelper.getHelper(_currentGraphics!).page!)
                          .section!)
                  .indexOf(PdfGraphicsHelper.getHelper(_currentGraphics!).page!)
              : PdfGraphicsHelper.getHelper(_currentGraphics!).page!.defaultLayerIndex;
      if (!PdfGridHelper.getHelper(_grid!).listOfNavigatePages.contains(index)) {
        PdfGridHelper.getHelper(_grid!).listOfNavigatePages.add(index);
      }
    }
    ...
    if (_grid!.rows.count != 0) {
      _currentBounds.width = (param.bounds!.width > 0)
          ? param.bounds!.width
          : (_currentBounds.width - _grid!.rows[0].cells[0].style.borders.left.width / 2);
    } else if (_grid!.headers.count != 0) {
      _currentBounds.width = (param.bounds!.width > 0)
          ? param.bounds!.width
          : (_currentBounds.width - _grid!.headers[0].cells[0].style.borders.left.width / 2);
    }
    _startLocation = param.bounds!.location;
    if (_grid!.style.allowHorizontalOverflow &&
        _currentBounds.width > _currentGraphics!.clientSize.width) {
      _currentBounds.width = _currentGraphics!.clientSize.width - _currentBounds.x;
    }
    if (PdfGridHelper.getHelper(_grid!).isChildGrid!) {
      _childHeight = param.bounds!.height;
    }
    if (param.format != null && PdfLayoutFormatHelper.isBoundsSet(param.format!)) {
      if (param.format!.paginateBounds.height > 0) {
        _currentBounds.height = param.format!.paginateBounds.height;
      }
    } else if (param.bounds!.height > 0 && !PdfGridHelper.getHelper(_grid!).isChildGrid!) {
      _currentBounds.height = param.bounds!.height;
    }
    if (PdfGridHelper.getHelper(_grid!).isChildGrid!) {
    ...
    if (PdfGridHelper.getHelper(_grid!).hasRowSpan) {
      for (int i = 0; i < _grid!.rows.count; i++) {
        ...
        if (!PdfGridRowHelper.getHelper(_grid!.rows[i]).isRowHeightSet) {
          ...
          PdfGridRowHelper.getHelper(_grid!.rows[i]).isRowSpanRowHeightSet = true;
        }
      }
    }
