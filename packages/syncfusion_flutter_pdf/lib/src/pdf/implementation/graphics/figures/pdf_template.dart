...
class PdfTemplate implements IPdfWrapper {
  ...
  PdfTemplate(double width, double height) {
    ...
    _helper.content[PdfDictionaryProperties.type] = PdfName(PdfDictionaryProperties.xObject);
    _helper.content[PdfDictionaryProperties.subtype] = PdfName(PdfDictionaryProperties.form);
  }
  ...
  PdfTemplate._fromPdfStream(PdfStream template) {
    ...
    final IPdfPrimitive obj =
        PdfCrossTable.dereference(_helper.content[PdfDictionaryProperties.bBox])!;
    final PdfRectangle rect = (obj as PdfArray).toRectangle();
    _size = rect.size;
    _helper.isReadonly = true;
  }

  PdfTemplate._(Offset origin, Size size, List<int> stream, PdfDictionary resources,
      bool isLoadedPage, PdfPageHelper page)
      : super() {
    _helper = PdfTemplateHelper(this);
    if (size == Size.zero) {
      throw ArgumentError.value(size, 'size', 'The size of the new PdfTemplate cannot be empty.');
    }
    _helper.content = PdfStream();
    ...
    _helper.content[PdfDictionaryProperties.resources] = PdfDictionary(resources);
  ...
  void _setSize(double width, double height, [Offset? origin]) {
    if (origin != null) {
      final PdfArray array = PdfArray(<double>[origin.dx, origin.dy, width, height]);
      ...
      _helper.content[PdfDictionaryProperties.bBox] = PdfArray.fromRectangle(rectangle);
      _size = PdfSize(width, height);
    }
  }
...
class PdfTemplateHelper {
  ...
  /// internal method
  static PdfTemplate internal(Offset origin, Size size, List<int> stream, PdfDictionary resources,
      bool isLoadedPage, PdfPageHelper page) {
  ...
  void cloneResources(PdfCrossTable? crossTable) {
    if (_resources != null && crossTable != null) {
      ...
      final PdfDictionary? resourceDict = _resources!.cloneObject(crossTable) as PdfDictionary?;
